...
namespace DialogueEditor.Dialogue.Editor
{
    public class ChoiceConnectorNode : BaseNode
    {
        ...
        public List<Node> lastChoiceNode = new List<Node>();
            ...
            if (portEdge.Any())
            {
                ...
                if (nodeGuid != null)
                {
                    ...
                    if (choiceNode != null)
                    {
                        ...
                        lastChoiceNode.Remove(choiceNode);
                    }
                }
            }
        ...
        {
            Vector2 returnPosition = GetPosition().position + new Vector2(GetPosition().width + 25, 0);
            if (lastChoiceNode.Count > 0)
            {
                returnPosition = lastChoiceNode.Last().GetPosition().position + new Vector2(0, lastChoiceNode.Last().GetPosition().height + 25);
            }
        ...
        {
            for (int i = 0; i < lastChoiceNode.Count; i++)
            {
                Vector2 newPosition;
                if(i == 0)
                    newPosition = GetPosition().position + new Vector2(GetPosition().width + 25, 0);
                else
                    newPosition = lastChoiceNode[i - 1].GetPosition().position + new Vector2(0, lastChoiceNode[i - 1].GetPosition().height + 25);
                lastChoiceNode[i].SetPosition(new Rect(newPosition, lastChoiceNode[i].GetPosition().size));
            }
        }
    }
}